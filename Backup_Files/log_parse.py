...
def extract_sensor_info(sensor_line, datestamp):
    sensor_info = [[f.split(';')[4].split(': ')[0], f.split(';')[
        4].split(': ')[1]] for f in sensor_line]

    sensor_timestamp = sensor_info[0][0]
    sensor_time = float(sensor_info[0][1])

    sensor_list = [datestamp, sensor_timestamp, sensor_time]
    return sensor_list
    ...
    for log in log_file_dir:
        ...
        while log_index < len(log_lines):
            ...
            if sensor_key in log_lines[log_index]:
                sensor_1 = extract_sensor_info(log_lines[log_index+1], log_datestamp)
                sensor_2 = extract_sensor_info(log_lines[log_index+2], log_datestamp)
                sensor_3 = extract_sensor_info(log_lines[log_index+3], log_datestamp)
                # datestamp = log_lines[log_index].split(';')[0]
                # sensor_check_lines = [log_lines[log_index], log_lines[log_index+1], log_lines[log_index+2]]
                # sensor_list = extract_sensor_info(sensor_check_lines, datestamp)
        ...
        if sensor_df_list:
            try:
                for i in range(1,4):
                    print(sensor_list[i-1])
                    new_log_title = log_title + '_' + str(i)
                    make_csv_and_graph(sensor_list[i-1], 'Proximity Sensor ' + str(i), os.path.join(sensor_dir_in, log_title), new_log_title)
            except Exception:
                print(sensor_df_list)
